...
const API_SERVER = "http://10.0.2.16:3000"; // Change to your machine's IP
...
Java.perform(function () {
      ...
      ApiInterceptor.intercept.implementation = function (chain) {
        ...
        if (reqBody) {
          try {
            ...
            var okioBuffer = null;
            try {
              okioBuffer = Java.use("okio.Buffer").$new();
            } catch (e) {
              // okio.Buffer not found, ignore request body
            }
            if (okioBuffer) {
              reqBody.writeTo(okioBuffer);
              var reqBodyStr = okioBuffer.readUtf8();
              console.log("Request Body: " + reqBodyStr.substring(0, 500));
            }
          } catch (e) {
            console.log("Request Body: [unable to read] " + e);
          }
        }
      ...
      try {
        ...
        for (var m = 0; m < methods.length; m++) {
          ...
          if (paramCount >= 1 && paramCount <= 3) {
            console.log(
              "[*] Found candidate encrypt method: " +
              encryptorClasses[i] +
              "." +
              methodName +
              " (params: " +
  ...
  try {
    ...
    Gson.toJson.overload("java.lang.Object").implementation = function (obj) {
      ...
      if (json.length > 50 && json.length < 10000) {
        ...
        if (
          className.includes("startshorts") ||
          className.includes("netshort")
        ) {
  ...
  // ============================================================
  try {
    var GsonConverterFactory = Java.use(
      "retrofit2.converter.gson.GsonResponseBodyConverter",
    );
    GsonConverterFactory.convert.overload("okhttp3.ResponseBody").implementation = function (body) {
      try {
  ...
  // ============================================================
  // HOOK 5: Generic OkHttp Call execution
  // ============================================================
  try {
    var RealCall = Java.use("okhttp3.RealCall");
    RealCall.execute.implementation = function () {
      var request = this.request();
      console.log("\n[OkHttp RealCall.execute] URL: " + request.url().toString());
      var response = this.execute();
      console.log("[OkHttp RealCall.execute] Status: " + response.code());
      return response;
    };
    RealCall.enqueue.implementation = function (callback) {
      var request = this.request();
      console.log("\n[OkHttp RealCall.enqueue] URL: " + request.url().toString());
  ...
  // ============================================================
  // HOOK 6: java.net.HttpURLConnection (fallback)
  // ============================================================
  try {
    ...
    HttpURLConnectionImpl.getOutputStream.implementation = function () {
      console.log("\n[HttpURLConnection] POST URL: " + this.getURL().toString());
      return this.getOutputStream();
    };
    HttpURLConnectionImpl.getInputStream.implementation = function () {
      console.log("\n[HttpURLConnection] GET Response for: " + this.getURL().toString());
