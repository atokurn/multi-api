 ...
 * ========================================
 * 
 * Business logic for FreeReels API operations.
 * Handles home feed, drama details, episodes, and video streams.
 * 
 * Based on reverse engineering of FreeReels APK.
 */

import * as freereelsClient from '../lib/freereelsClient.js';
...
async function ensureAuthenticated() {
    if (!hasAttemptedDynamicLogin) {
        ...
        console.log('[FreeReelsService] Attempting fresh anonymous login for dynamic token...');
        try {
            const result = await freereelsClient.anonymousLogin();
            if (result.success) {
                console.log('[FreeReelsService] Dynamic token obtained successfully');
            } else {
                console.log('[FreeReelsService] Dynamic login failed, using static token:', result.error);
            }
        } catch (err) {
            console.log('[FreeReelsService] Dynamic login error, using static token:', err.message);
        }
    }
    // Ensure we have at least the static token
    if (!freereelsClient.isAuthenticated()) {
...
async function getForYou(page = 1, limit = 20) {
    ...
    try {
        const response = await freereelsClient.get('/foryou/feed', {
            page,
            page_size: limit
        });
        ...
        console.error('[FreeReelsService] getForYou failed:', error.message);
        throw error;
    }
}
...
// Based on sapimu.au FreeReels API reference
// ============================================
...
async function getNewReleases(page = 1, limit = 20) {
    ...
    try {
        ...
        const response = await freereelsClient.post('/homepage/v2/rank', {
            key: 'new'
        }).catch(() => null);
        ...
        console.error('[FreeReelsService] getNewReleases failed:', error.message);
        return getForYou(page, limit);
    }
}
...
async function getFreeDramas(page = 1, limit = 20) {
    ...
    try {
        ...
        console.error('[FreeReelsService] getFreeDramas failed:', error.message);
...
async function getVipDramas(page = 1, limit = 20) {
    ...
    try {
        ...
        const response = await freereelsClient.post('/homepage/vip_series', {})
            .catch(() => null);
        ...
        console.error('[FreeReelsService] getVipDramas failed:', error.message);
        return { success: false, error: error.message, data: [] };
    }
}
...
async function getDubbingDramas(page = 1, limit = 20) {
    ...
    try {
        ...
        console.error('[FreeReelsService] getDubbingDramas failed:', error.message);
        return { success: false, error: error.message, data: [] };
    }
}
...
async function getMaleDramas(page = 1, limit = 20) {
    ...
    try {
        ...
        console.error('[FreeReelsService] getMaleDramas failed:', error.message);
        return getForYou(page, limit);
    }
}
...
async function getFemaleDramas(page = 1, limit = 20) {
    ...
    try {
        ...
        console.error('[FreeReelsService] getFemaleDramas failed:', error.message);
        return getForYou(page, limit);
    }
}
...
async function getComingSoon(page = 1, limit = 20) {
    await ensureAuthenticated();
    try {
        const response = await freereelsClient.get('/coming-soon/list', {
            page,
            page_size: limit
        }).catch(() => null);
        ...
        console.error('[FreeReelsService] getComingSoon failed:', error.message);
        return { success: false, error: error.message, data: [] };
    }
}
...
async function getSearchHot() {
    await ensureAuthenticated();
    try {
        const response = await freereelsClient.get('/search/hot_words')
            .catch(() => null);
        ...
        console.error('[FreeReelsService] getSearchHot failed:', error.message);
        return { success: false, error: error.message, data: [] };
    }
}
...
async function playEpisodeByIndex(seriesId, episodeIndex = 1) {
    ...
    try {
        ...
        if (cachedEp && cachedEp.videoUrl) {
            console.log(`[FreeReelsService] Cache HIT for episode ${seriesId}:${episodeIndex}`);
            return {
                success: true,
                data: {
        ...
        try {
            const infoResponse = await freereelsClient.get('/drama/info_v2', {
                series_id: seriesId,
                scene: 'for_you'
            }).catch((err) => {
                console.error(`[FreeReelsService] info_v2 request thrown for ${seriesId}:`, err.message);
                if (err.response?.data) console.error(`[FreeReelsService] info_v2 error data:`, err.response.data);
                return null;
            });

            if (infoResponse) {
                console.log(`[FreeReelsService] info_v2 response keys for ${seriesId}:`, Object.keys(infoResponse));
                if (infoResponse.data) {
                    console.log(`[FreeReelsService] info_v2 response.data keys:`, Object.keys(infoResponse.data));
                }
            }
            ...
            if (infoResponse && (infoResponse.code === 200 || infoResponse.code === 0) && infoResponse.data) {
                ...
                if (episodeList.length > 0) {
                    console.log(`[FreeReelsService] info_v2 parsed ${episodeList.length} episodes for ${seriesId}`);
                    ...
                    console.log(`[FreeReelsService] Search for index ${episodeIndex} returned:`, !!episode, 'videoUrl:', !!episode?.videoUrl);
            ...
            console.log('[FreeReelsService] info_v2 failed:', infoErr.message);
        }
        ...
        console.error('[FreeReelsService] playEpisodeByIndex failed:', error.message);
...
async function search(keyword, page = 1, limit = 20) {
    ...
    try {
        ...
        const response = await freereelsClient.post('/search/drama', {
            keyword: keyword,
        ...
        console.error('[FreeReelsService] search failed:', error.message);
...
async function getDetail(seriesId) {
    ...
    try {
        ...
        if (cached) {
            console.log('[FreeReelsService] Found drama in cache:', cached.title);
            return {
                success: true,
                data: cached,
                source: 'cache'
        ...
        const response = await freereelsClient.get('/drama/info_v2', {
            series_id: seriesId,
            scene: 'for_you'
        }).catch(() => null);
        ...
        console.log('[FreeReelsService] info_v2 empty, refreshing forYou for:', seriesId);
        ...
        if (forYouResult.success && forYouResult.data && forYouResult.data.length > 0) {
            ...
            if (found) {
                console.log('[FreeReelsService] Found drama in fresh forYou:', found.title);
                return {
                    success: true,
                    data: found,
                    source: 'foryou'
        ...
        // NOT FOUND anywhere
        console.log('[FreeReelsService] Drama not found:', seriesId);
        return {
            success: false,
        ...
        console.error('[FreeReelsService] getDetail failed:', error.message);
        return { success: false, error: error.message, data: null };
    }
}
...
async function getEpisodes(seriesId, page = 1, limit = 100) {
    ...
    try {
        ...
        console.error('[FreeReelsService] getEpisodes failed:', error.message);
        throw error;
    }
}
...
async function getVideo(episodeId) {
    ...
    try {
        ...
        if (cached && cached.videoUrl) {
            console.log('[FreeReelsService] Found episode in cache:', cached.name);
            return {
                success: true,
                data: {
                    episodeId: episodeId,
        ...
        const response = await freereelsClient.get('/drama/episode', {
        ...
        console.error('[FreeReelsService] getVideo failed:', error.message);
        return { success: false, error: error.message, data: null };
    }
}
